...
#ifndef _LOTTERYSTATION_HPP_
#define _LOTTERYSTATION_HPP_
...
public:
    ...
    // If succeeds, output a message: "Bought a (??) ticket for (??) Yuan at round
    ...
    // You should not claim a ticket that's already claimed or with wrong
    // rounds. In either case above, simply print: "This ticket wins 0 Yuan.\n"
    ...
    // results. It sets the prize numbers of the type given as "numbers".
    // "tickettype" is either LOTTO6 or LOTTO7 (predefined).
    // If "numbers" are invalid (not enough/out of bounds), do nothing and
    // return false. If succeeded, return true.

private:
    int m_round;  // Should be initialized to 0.
    CustomSet<int> m_prizeNumberfor6;
    CustomSet<int> m_prizeNumberfor7;
};
